...
def test_test_method(sample_df, spark):
    ...
    assert "primary_key" in result_df.columns
    assert "test_name" in result_df.columns
    assert "test_col" in result_df.columns
    assert "test_result" in result_df.columns
    assert result_df.filter(result_df["test_result"]).count() == 3
...
def test_test_method_with_filter(sample_df, spark):
    ...
    assert "test_col" in result_df.columns
    assert "test_result" in result_df.columns
    assert result_df.filter(result_df["test_result"]).count() == 2
...
def test_test_method_with_context_cols(sample_df, spark):
    ...
    # The returned result from .test should include context columns
    assert "name" in result_df.columns
    # The internal results should also keep the context column
...
def test_test_method_with_dummy_run(sample_df, spark):
    ...
    assert "test_result" in result_df.columns
    assert result_df.filter(result_df["test_result"]).count() == 3
    assert tester.results.count() == 0  # No results should be saved in dummy run
...
def test_add_custom_test_result(sample_df, spark):
    ...
    assert "primary_key" in updated_results.columns
    assert "test_name" in updated_results.columns
    assert "test_col" in updated_results.columns
    assert "test_result" in updated_results.columns
    # Check that the custom test was added with correct description
    custom_test_rows = updated_results.filter(
        col("test_name") == "custom_test"
    ).collect()
    assert len(custom_test_rows) > 0
    assert custom_test_rows[0]["test_description"] == "This is a custom test"
    assert updated_results.filter(col("test_result")).count() == 2
    assert updated_results.filter(~col("test_result")).count() == 2
...
def test_add_custom_test_result_with_context_cols_and_return_extra(sample_df, spark):
    ...
    # Should include the context column and requested return extra col
    assert "primary_key" in updated_results.columns
    ...
    # The internal results should retain the context column as well
    assert "name" in tester.results.columns
...
def test_add_custom_test_result_with_value_column(sample_df, spark):
    """Test custom test result with value column functionality."""
    tester = DataFrameTester(df=sample_df, primary_key="id", spark=spark)

    # Create custom test data with a value column
    custom_test_data = [
        (1, True, "test_value_1"),
        (2, False, "test_value_2"),
        (3, True, "test_value_3"),
        (4, False, "test_value_4"),
    ]
    custom_test_columns = ["id", "custom_test", "test_values"]
    custom_test_result = spark.createDataFrame(custom_test_data, custom_test_columns)

    updated_results = tester.add_custom_test_result(
        result=custom_test_result,
        name="custom_test",
        description="Custom test with value column",
        value_column="test_values",
    )

    # Check that the test_value column contains the actual values instead of "N/A"
    custom_test_rows = updated_results.filter(
        col("test_name") == "custom_test"
    ).collect()
    assert len(custom_test_rows) == 4

    # Verify that test_value contains the actual values from the value_column
    test_values = [row["test_value"] for row in custom_test_rows]
    assert "test_value_1" in test_values
    assert "test_value_2" in test_values
    assert "test_value_3" in test_values
    assert "test_value_4" in test_values
    assert "N/A" not in test_values


def test_add_custom_test_result_with_value_column_not_found(sample_df, spark):
    """Test that ValueError is raised when value_column is not found in result DataFrame."""
    tester = DataFrameTester(df=sample_df, primary_key="id", spark=spark)
    ...
    with pytest.raises(
        ValueError,
        match="value_column 'nonexistent_column' not found in result DataFrame",
    ):
        tester.add_custom_test_result(
            result=custom_test_result,
            name="custom_test",
...
def test_summary(sample_df, spark):
    ...
    assert test_summary["n_passed"] == 4
    assert test_summary["n_failed"] == 1
    assert test_summary["percentage_passed"] == 80.0
    assert test_summary["percentage_failed"] == 20.0
    assert test_summary["timestamp"] is not None
...
def test_summary_after_filter(sample_df, spark):
    ...
    assert test_summary["n_passed"] == 3
    assert test_summary["n_failed"] == 1
    assert test_summary["percentage_passed"] == 75.0
    assert test_summary["percentage_failed"] == 25.0
